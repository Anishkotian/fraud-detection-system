...
def fraud_pie_chart(df):
    ...
    fig = go.Figure(go.Pie(
        ...
        values=values,
        hole=0.6,
        marker=dict(colors=[COLORS["legit"], COLORS["fraud"]], line=dict(color=COLORS["bg"], width=3)),
        textfont=dict(size=14, color="white"),
    ))
    fig.add_annotation(text=f"<b>{values[1]}</b><br>Frauds", x=0.5, y=0.5, showarrow=False,
...
def amount_distribution(df):
    ...
    fig = go.Figure()
    fig.add_trace(go.Histogram(x=legit, name="Legitimate", opacity=0.7, marker_color=COLORS["legit"], nbinsx=50))
    fig.add_trace(go.Histogram(x=fraud, name="Fraud", opacity=0.7, marker_color=COLORS["fraud"], nbinsx=50))
    fig.update_layout(**_base_layout(
        barmode="overlay",
        title="Transaction Amount Distribution by Class",
        xaxis=dict(title="Amount ($)", gridcolor=COLORS["grid"]),
...
def fraud_by_category(df, col="merchant_category"):
    ...
    fig.update_layout(**_base_layout(
        ...
        xaxis=dict(title="Fraud Rate (%)", gridcolor=COLORS["grid"]),
...
def confusion_matrix_heatmap(cm_data, model_name):
    cm = np.array(cm_data)
    ...
    fig.update_layout(**_base_layout(
        ...
        xaxis=dict(side="bottom", gridcolor=COLORS["grid"]),
        yaxis=dict(gridcolor=COLORS["grid"]),
...
def feature_importance_chart(importances):
    features = importances["features"][:10]
    values = importances["importances"][:10]
    fig = go.Figure(go.Bar(
        x=values[::-1],
        y=features[::-1],
        orientation="h",
        marker=dict(color=values[::-1], colorscale=[[0, COLORS["secondary"]], [1, COLORS["primary"]]]),
...
def probability_gauge(probability, model_name):
    ...
    fig = go.Figure(go.Indicator(
        ...
        title=dict(text=f"Fraud Probability<br><span style='font-size:12px'>{model_name}</span>", font=dict(size=14)),
        gauge=dict(
...
def scatter_amount_vs_risk(df_results):
    ...
    fig.update_layout(**_base_layout(
        ...
        xaxis=dict(title="Transaction Amount ($)", gridcolor=COLORS["grid"]),
        yaxis=dict(title="Fraud Probability", gridcolor=COLORS["grid"]),
        legend=dict(orientation="h", y=1.1),
        margin=dict(t=60, b=40, l=50, r=20),
        height=380
    ))
